...
layout ( std430, binding = 0 ) volatile buffer RaysSSBO { RayRework nodes[]; } rayBuf;
layout ( std430, binding = 1 ) volatile buffer HitsSSBO { HitRework nodes[]; } hitBuf;

#ifndef SIMPLIFIED_RAY_MANAGMENT
layout ( std430, binding = 2 ) volatile buffer TexelsSSBO { Texel nodes[]; } texelBuf;
layout ( std430, binding = 3 ) volatile buffer ColorChainBlock { ColorChain chains[]; } chBuf;
#endif
...
// new list
#ifndef SIMPLIFIED_RAY_MANAGMENT
layout ( std430, binding = 6 ) volatile buffer CollectedActivesSSBO { int indc[]; } collBuf;
layout ( std430, binding = 7 ) volatile buffer FreedomIndicesSSBO { int indc[]; } freedBuf;
#endif

// counters
layout ( std430, binding = 8 ) volatile buffer CounterBlock { 
