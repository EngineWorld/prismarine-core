...
namespace Paper {
    ...
    inline void Intersector::allocate(const size_t &count) {
        ...
        glNamedBufferStorage(mortonBuffer, strided<int32_t>(maxt * 1), nullptr, GL_DYNAMIC_STORAGE_BIT);

        glCreateBuffers(1, &mortonBufferIndex);
        glNamedBufferStorage(mortonBufferIndex, strided<int32_t>(maxt * 1), nullptr, GL_DYNAMIC_STORAGE_BIT);
        ...
        glNamedBufferStorage(leafBufferSorted, strided<Leaf>(maxt * 1), nullptr, GL_DYNAMIC_STORAGE_BIT);

        glCreateBuffers(1, &leafBuffer);
        glNamedBufferStorage(leafBuffer, strided<Leaf>(maxt * 1), nullptr, GL_DYNAMIC_STORAGE_BIT);
