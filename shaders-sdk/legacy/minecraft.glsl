    ...
    float grefl = 0.2f + pow((1.0f - abs(dot(normal, newRay.direct.xyz))), 2.0f) * 0.6f;
     float glossiness = clamp(1.0f / log2(materialCache.reflectivity), 0.0f, 1.0f);
     bool isWater = iseq16(hit.vmods.x, 8.f) || iseq16(hit.vmods.x, 9.f);
     bool isGlass = iseq16(hit.vmods.x, 20.f) || iseq16(hit.vmods.x, 102.f);
     bool isStained = iseq16(hit.vmods.x, 95.f) || iseq16(hit.vmods.x, 160.f);
    ...
    if (isWater) {
         float frameTimeCounter = float(double(RAY_BLOCK materialUniform.iModifiers0.w) / 60000.0f) * 4.0f;
        ...
        Ray refrRay = refraction(newRay, hit, vec3( 1.0f ), normal, inior, outior, 0.0f);
         float fres = computeFresnel(normal, newRay.direct.xyz, inior, outior) * (inside <= 0.0f ? 1.0f : 0.0f);
