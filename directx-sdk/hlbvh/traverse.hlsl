...
float3 projectVoxels(in float3 orig) {
     float4 nps = mul(float4(orig, 1.0f), GEOMETRY_BLOCK octreeUniform.project);
    return nps.xyz / nps.w;
}

float3 unprojectVoxels(in float3 orig) {
     float4 nps = mul(float4(orig, 1.0f), GEOMETRY_BLOCK octreeUniform.unproject);
    return nps.xyz / nps.w;
}
...
TResult traverse(in uint L, in float distn, in float3 origin, in float3 direct, in Hit hit) {
    ...
    // test constants
     ...
     float3 tdirproj = mul(float4(direct, 0.0), GEOMETRY_BLOCK octreeUniform.project).xyz;
